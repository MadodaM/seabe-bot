...
module.exports = function(app, upload, { prisma, syncToHubSpot }) {
   ...
   // ==========================================
    ...
    // ==========================================
    // 🔔 GLOBAL PAYMENT WEBHOOK (Netcash NIWS)
    // ==========================================
    app.post('/api/webhooks/payment-success', express.json(), async (req, res) => {
        // Netcash payload structure (abstracted for JSON)
        const { status, reference, token, cardType, maskedCard, enrollmentId } = req.body; 

        if (status === 'SUCCESS') {
            try {
                // ------------------------------------------------
                // SCENARIO A: LMS Course Payment
                // ------------------------------------------------
                if (enrollmentId) {
                ...
                // ------------------------------------------------
                // SCENARIO B: Society Premium Payment (SEABE ID VAULT)
                // ------------------------------------------------
                if (reference && reference.includes('-ONCEOFF-')) {
                    // Parse reference: e.g., TES624-ONCEOFF-2707-168234
                    const parts = reference.split('-');
                    const orgCode = parts[0];
                    const phoneSuffix = parts[2];

                    // Find the member using the org code and the last 4 digits of their phone
                    ...
                    if (member && token) {
                        ...
                        if (!existingToken) {
                            ...
                            console.log(`💳 [SEABE ID] Vaulted new ${cardType} for Member #${member.id}`);
                            
                            // Send a "Card Saved" WhatsApp Notification
                            if (process.env.TWILIO_SID) {
                                ...
                                await twilioClient.messages.create({
                                    ...
                                    body: `✅ *Payment Successful & Card Saved!*\n\nFor faster checkouts in the future, we've securely linked your *${cardType || 'Card'} ending in ${maskedCard ? maskedCard.slice(-4) : '****'}* to your Seabe ID.\n\nReply *Menu* to return to your dashboard.`
                                });
                            }
                        }
                    } else if (member && !token) {
                        // Just a successful payment without a token (e.g., EFT)
                        if (process.env.TWILIO_SID) {
                            const twilioClient = require('twilio')(process.env.TWILIO_SID, process.env.TWILIO_AUTH);
                            const cleanTwilioNumber = process.env.TWILIO_PHONE_NUMBER.replace('whatsapp:', '');
                            await twilioClient.messages.create({
                                from: `whatsapp:${cleanTwilioNumber}`,
                                to: `whatsapp:${member.phone}`,
                                body: `✅ *Payment Successful!*\n\nWe have received your premium payment. Your policy is up to date.\n\nReply *Menu* to return to your dashboard.`
                            });
                        }
                    }
                }
