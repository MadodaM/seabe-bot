...
module.exports = function(app, upload, { prisma, syncToHubSpot }) {
    ...
    const whatsAppButton = `
        <a href="https://wa.me/27832182707" class="whatsapp-float" target="_blank">
	...
	const cookieBanner = `
        <div id="cookie-banner" class="fixed bottom-0 left-0 w-full bg-slate-900 border-t border-slate-800 text-white p-4 z-[100] transform translate-y-full transition-transform duration-500 ease-in-out">
            <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="text-sm text-slate-400">
                    <strong class="text-white">We use cookies.</strong> This platform uses necessary cookies to secure your sessions and enhance your browsing experience. By clicking "OK", you consent to our use of cookies. <a href="/privacy" class="text-seabe-teal hover:underline font-bold">Read our Privacy Policy</a>.
                </div>
                <button id="accept-cookies" class="bg-seabe-teal text-white font-bold py-2 px-8 rounded-lg hover:bg-teal-500 transition shadow-lg whitespace-nowrap">
                    OK
                </button>
            </div>
        </div>
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                // If they haven't accepted yet, slide the banner up
                if (!localStorage.getItem('seabe_cookie_consent')) {
                    setTimeout(() => banner.classList.remove('translate-y-full'), 500);
                }
                
                // When they click OK, save to browser and slide down
                btn.addEventListener('click', () => {
                    localStorage.setItem('seabe_cookie_consent', 'true');
